...
`dsh-dock-host` - a generic **dock-button host** for a dsh (DeepSeek Harness)
web client, built as a Feature-Sliced Design static Cordis plugin. It is
**client-only**: the host half (`src/index.ts`) registers no tools and no RPC;
all dock capabilities (button registry, `conversation.input.dock` row renderer,
overflow menu, i18n, motion) live in `src/client/`.
...
- **Client-only**: this plugin has no host Remote. Do not add a Typert Remote
  or a host capability unless a feature genuinely needs one. Keep the host root
  a no-op.
